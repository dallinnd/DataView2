...
function renderBoxControls() {
    ...
    if (!box.isVar) {
        contentSection = `<input type="text" value="${box.textVal}" oninput="syncBoxAttr(${selectedBoxIdx}, 'textVal', this.value, false)">`;
    } else if (hasData) {
        ...
        contentSection = `
            ...
            <div class="pills-container" id="pills-box">
                ${filtered.map(h => `<div class="var-pill ${box.textVal === h ? 'selected' : ''}" onclick="syncBoxAttr(${selectedBoxIdx}, 'textVal', '${h}', true)">${h}</div>`).join('')}
            </div>`;
    } else {
        contentSection = `<button class="orange-btn" style="width:100%; font-size:0.8rem;" onclick="uploadExcel()">Upload Excel Data First</button>`;
    }
